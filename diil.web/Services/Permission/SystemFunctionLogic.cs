...
namespace EIP.System.Business.Permission
{
    ...
    /// 功能项业务逻辑
    /// </summary>
    public class SystemFunctionLogic : AsyncLogic<SystemFunction>, ISystemFunctionLogic
    {
        #region 构造函数
        ...
        public async Task<OperateStatus> SaveFunction(IList<MvcRote> rotes)
        {
            ...
            if (rotes.Any())
            {
                try
                {
                    ...
                    //新增
                    if (insertFunctions.Any() && await _functionRepository.InsertMultipleAsync(insertFunctions) > 0)
                    {
                        operateStatus.ResultSign = ResultSign.Successful;
                    }
