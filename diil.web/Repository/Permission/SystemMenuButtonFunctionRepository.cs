...
namespace EIP.System.DataAccess.Permission
{
    ...
    public class SystemMenuButtonFunctionRepository : DapperAsyncRepository<SystemMenuButtonFunction>, ISystemMenuButtonFunctionRepository
    {
        ...
        {
            const string sql = @"SELECT func.*,app.Code AppCode FROM dbo.System_MenuButtonFunction buttonfunc
                                            LEFT JOIN dbo.System_Function func ON func.FunctionId = buttonfunc.FunctionId
                                            LEFT JOIN dbo.System_App app ON app.Code=func.AppCode
        ...
        /// <param name="input"></param>
        /// <returns></returns>
        public Task<IEnumerable<SystemMenuButtonFunctionOutput>> GetAllFunctions(GetAllFunctionsInput input)
        {
            StringBuilder sql =
                new StringBuilder(@"SELECT func.*,app.Code AppCode FROM dbo.System_Function func LEFT JOIN dbo.System_App app ON app.Code=func.AppCode");
        ...
        public Task<bool> DeleteMenuButtonFunction(SystemMenuButtonFunction menuButtonFunction)
        {
            ...
            return SqlMapperUtil.InsertUpdateOrDeleteSqlBool<SystemMenuButtonFunction>(sql.ToString(), new
            {
